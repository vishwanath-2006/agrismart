...
export interface AccordionGalleryProps {
  ...
  grayscale?: boolean;
  className?: string;
  onItemClick?: (index: number, item: AccordionGalleryItem) => void;
  onChange?: (index: number, item: AccordionGalleryItem) => void;
}
    ...
    (animate: boolean) => {
      ...
      panels.forEach((panel, i) => {
        ...
        if (showLabels && bar && text) {
          const labelsToAnimate = subtext ? [bar, text, subtext] : [bar, text];
          if (isActive) {
            tl.to(labelsToAnimate, { opacity: 1, x: 0, duration: dur, ease, stagger: prefersReduced ? 0 : stagger }, 0);
          } else {
            tl.to(labelsToAnimate, { opacity: 0, x: -14, duration: dur * 0.6, ease }, 0);
          }
        }
      });
          ...
          >
            ...
            {item.badge && (
              <div className="ag-panel__badge">
                <span className="bg-[#0f5238] text-white px-2.5 py-1 rounded-full text-[11px] font-bold shadow-md flex items-center gap-1 border border-white/20">
